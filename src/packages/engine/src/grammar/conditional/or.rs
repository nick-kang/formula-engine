...
pub fn evaluate(pairs: Pairs<Rule>, data: &HashMap<String, Value>) -> Result<Primative, Error> {
    let booleans = and_or_util(pairs, data, String::from("or"))?;

    Ok(evaluate_raw(booleans))
}

fn evaluate_raw(booleans: Vec<bool>) -> Primative {
    Primative::Boolean(booleans.contains(&true))
}
...
mod tests {
    ...
    fn it_returns_true() {
        ...
        assert_eq!(Primative::Boolean(true), evaluate_raw(vec![true, false]));
    }

    #[test]
    fn it_returns_false() {
        assert_eq!(Primative::Boolean(false), evaluate_raw(vec![false]));
        assert_eq!(Primative::Boolean(false), evaluate_raw(vec![false, false]));
    }
}
